...
type ScriptNodeType = Node<ScriptNodeData, "scriptNode">;

const DEFAULT_SCRIPT = "function edit(data) {}";

const ScriptNode: React.FC<NodeProps<ScriptNodeType>> = ({ data, isConnectable }) => {
  ...
  const { bgColor, borderColor } = useMemo(() => {
    ...
    if (data?.isDone === null && data?.isError === null ) {
       return { bgColor: "bg-gray-900", borderColor: "border-purple-400" };
    }
    return { bgColor: "bg-gray-900", borderColor: "border-purple-400" };
  ...
  return (
    <>
      ...
      >
        <Stack gap="md">
          <Textarea
            label="Script to return a new object"
